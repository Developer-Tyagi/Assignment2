    ...
    <div class="vendor-list">
        ...
        <div>
            <div v-for="vendor in vendors" :key="vendor.id" class="vendor-list-item" @click="selectVendor(vendor)">
                <span>{{ vendor.name }}</span> <q-icon v-if="vendor.selected" name="done" size="xs" class="q-ml-auto"/>
            </div>
        </div>
        <q-dialog
        ...
        >
            <q-card class="bg-white">
                <q-bar style="height:51px;border-bottom: 1px solid #0000001f;" class="bg-white">
                <img src="~assets/close.svg" alt="close" @click="filterDialog = false" />
                <q-space />
                <div class="text-uppercase text-bold text-black" >
                    Filters
                </div>
                <q-space />
                <q-btn color="primary" text-color="white"  @click="applyFilter()">Apply</q-btn> 
                </q-bar>
                <div class="filters">
                <div v-for="filter in vendorIndustries" :key="filter.id" class="filters-list-item" @click="selectFilter(filter)">
                   <div class="row"> {{ filter.name }} <q-radio  v-model="selectedFilter" :val="filter.id"  dense class="q-ml-auto"/></div>
                </div>
            </div>
            </q-card>
        </q-dialog>
